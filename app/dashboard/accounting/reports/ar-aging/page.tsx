...
export default function ArAgingPage() {
  ...
  const fetchData = async () => {
    ...
    const res = await fetch(`${API_BASE_URL}/accounting/reports/ar-aging${params}`, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Accounts Receivable Aging</h1>
          <p className="text-sm text-gray-500 mt-1">Which customers owe money, and how overdue.</p>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="py-3 px-6 text-xs font-semibold text-gray-600 uppercase">Client</th>
                  ...
                  {data.rows.map((r: any) => (
                    <tr key={r.orderId} className="hover:bg-gray-50">
                      <td className="py-4 px-6">
                        <Link href={`/dashboard/sales-invoices/${r.orderId}`} className="font-bold text-purple-700 font-mono hover:underline">{r.invoiceNumber}</Link>
                      </td>
                      <td className="py-4 px-6 text-sm font-bold">{r.clientName}</td>
