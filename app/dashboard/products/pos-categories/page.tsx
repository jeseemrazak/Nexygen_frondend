...
export default function PosCategoriesPage() {
  ...
  const fetchAll = async () => {
    ...
    const res = await fetch(`${API_BASE_URL}/pos-categories`, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setError('');
    const token = getClientToken();
    const res = await fetch(`${API_BASE_URL}/pos-categories`, {
      method: 'POST',
  ...
  const toggleActive = async (cat: any) => {
    const token = getClientToken();
    const res = await fetch(`${API_BASE_URL}/pos-categories/${cat.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    <div className="max-w-3xl mx-auto space-y-6">
        ...
        <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-4">
          <div className="flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="e.g. Beverages, Snacks, Bakery"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">No POS categories yet.</div>
        ) : (
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="py-3 px-6 text-xs font-semibold text-gray-600 uppercase">Name</th>
                <th className="py-3 px-6 text-xs font-semibold text-gray-600 uppercase">Sort Order</th>
